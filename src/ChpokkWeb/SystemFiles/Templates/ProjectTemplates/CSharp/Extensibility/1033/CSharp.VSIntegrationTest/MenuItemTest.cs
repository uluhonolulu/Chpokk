...
namespace $safeprojectname$
{
    ...
    public class MenuItemTest
    {
        ...
        public void LaunchCommand()
        {
            ...
            {
                CommandID menuItemCmd = new CommandID($packageNamespace$.GuidList.$packageCmdSetGuid$, (int)$packageNamespace$.PkgCmdIDList.$cmdidMenuItem$);

                // Create the DialogBoxListener Thread.
                string expectedDialogBoxText = string.Format(CultureInfo.CurrentCulture, "{0}\n\nInside {1}.MenuItemCallback()", "$packageName$", "$packageNamespace$.$packageClass$Package");
