...
namespace Chpokk.Tests {
	...
	public class LessProblem {
		public LessProblem() {
			//CThruEngine.AddAspect(new TraceAspect(info => info.TypeName.Contains("AppDomain"), @"D:\" + AppDomain.CurrentDomain.FriendlyName + ".txt"));
		...
		public void CanHandleImport() {
			...
			//CThruEngine.AddAspect(new TraceAspect(info => info.TypeName.Contains("Exception")));
			CThruEngine.AddAspect(new NewTraceAspect(info => info.TargetInstance is AssetFileGraph));
			//CThruEngine.AddAspect(new TraceResultAspect(info => info.MethodName == "GetFullPath"));
			CThruEngine.AddAspect(new TraceAspect(info => info.TypeName.EndsWith("PathResolver")));
