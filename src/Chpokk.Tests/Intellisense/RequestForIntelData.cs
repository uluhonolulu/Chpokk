...
namespace Chpokk.Tests.Intellisense {
	...
	public class RequestForIntelData : WebQueryTest<ProjectWithSingleRootFileContext, IntelOutputModel> {
		...
		public override IntelOutputModel Act() {
			...
			var inputModel = new IntelInputModel {Content = text, Position = position, NewChar = '.', ProjectPath = projectPathRelativeToRepositoryRoot, RepositoryName = Context.REPO_NAME, PathRelativeToRepositoryRoot = "file.cs"};
